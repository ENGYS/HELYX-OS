...
public class VTKGeometry3DController implements Geometry3DController, Picker {
    ...
    @Override
    public void updateSurfacesFilter(Surface... surfaces) {
        logger.debug("[FILTER] {} surfaces filtered {}", surfaces.length, surfaces.length == 1 ? ", selection is: " + surfaces[0].getName() : "");

        List<Actor> selection = new ArrayList<Actor>();

        _updateSurfacesFilter(surfaces, selection);

        // if (selection.isEmpty()) {
        // logger.debug("updateSurfacesSelection: NONE selected!");
        // } else {
        if (!actorsMap.isEmpty()) {
            renderPanel.filterActors(selection.toArray(new Actor[0]));
        }
        // }
        
    }

    private void _updateSurfacesFilter(Surface[] surfaces, List<Actor> selection) {
        for (Surface surface : surfaces) {
            if (surface.hasRegions() && surface.isSingleton()) {
                _updateSurfacesFilter(surface.getRegions(), selection);
                continue;
            }
    ...
    protected Collection<Actor> getListOfSurfaceActors(Surface[] surfaces) {
    ...
    public void newContext(Class<? extends View3DElement> klass) {
        ...
        context.setActorsMap(getActorsMap());
