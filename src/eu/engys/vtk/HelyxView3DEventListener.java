...
import eu.engys.gui.events.view3D.RotatedBoxEvent;
		...
		SwingUtilities.invokeLater(new Runnable() {
			...
			public void run() {
				if (event instanceof PointEvent) {
					...
					view3D.showPoint(key, point, action, color);
					...
					DoubleField angle1 = e.getAxisInfo().getCamber();
					DoubleField angle2 = e.getAxisInfo().getToe();
					double magnitude = e.getAxisInfo().getMagnitude();
					int sign = e.getAxisInfo().getSign();
					EventActionType action = e.getAxisInfo().getAction();
					if (normal != null) {
					    view3D.showAxis(origin, normal, magnitude, action);
					} else {
					    view3D.showAxis(origin, angle1, angle2, magnitude, sign, action);
					}
				} else if (event instanceof PlaneEvent) {
					String  key = ((PlaneEvent) event).getKey();
                    ...
                    if (interactive) {
                        view3D.showPlane(key, origin, normal, action);
                    } else {
                        view3D.showPlaneDisplay(key, origin, normal, action);
                    }
                ...
                } else if (event instanceof RotatedBoxEvent) {
                    DoubleField[] center = ((RotatedBoxEvent) event).getCenter();
                    DoubleField[] delta = ((RotatedBoxEvent) event).getDelta();
                    DoubleField[] rotation = ((RotatedBoxEvent) event).getRotation();
                    EventActionType action = ((RotatedBoxEvent) event).getAction();
                    view3D.showRotatedBox(center, delta, rotation, action);
