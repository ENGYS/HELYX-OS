...
public class ReopenOnInitialised implements ServerListener {

    private Controller controller;

    public ReopenOnInitialised(Controller controller) {
        this.controller = controller;
    }

    @Override
    public void serverChanged(ServerState serverState) {
        if (serverState.getCommand().equals(Command.INITIALISE_FIELDS)) {
            if (serverState.getSolverState().isInitialised()) {
                if (controller.getListener() != null) {
                    controller.reopenCase(OpenMode.MESH_ONLY);
                }
            }
        }
    }
