...
public class ParsersHandler {
    ...
    public ParsersHandler(Model model, Set<ApplicationModule> modules) {
        this.model = model;
    ...
    public void deleteUselessLogFiles() {
        ...
        ModulesUtil.deleteUselessLogFiles(modules);
    }
    ...
    private void registerParsersForFunctionObject(String foName) {
        if (foName.equals(ResidualsParser.KEY)) {
            Parser residualsParser = ModulesUtil.createResidualsParser(modules, ResidualsUtils.fileToParse(model));
            if (residualsParser == null) {
                residualsParser = new SimpleResidualsParser(ResidualsUtils.fileToParse(model));
            }
        ...
        List<Parser> parsers = ModulesUtil.createParsers(modules);
        for (Parser parser : parsers) {
            register(parser);
        }
    }
