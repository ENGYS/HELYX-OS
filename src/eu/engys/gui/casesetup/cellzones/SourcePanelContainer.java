...
public class SourcePanelContainer extends JPanel {
        ...
        if (cellZone != null) {
            for (CellZoneType type : types) {
                if (cellZone.hasType(type.getKey()) && type.isEnabled()) {
                    showPanel(cellZone.getName(), type, cellZone.getDictionary(type.getKey()));
                } else {
                    hidePanel(type);
                }
            }
    ...
    public void showPanel(String cellZoneName, CellZoneType type, Dictionary cellZoneDictionary) {
        type.getPanel().loadFromDictionary(cellZoneName, cellZoneDictionary);
        ...
        for (CellZoneType type : types) {
            ...
            if (type.isEnabled()) {
                if (zone.getTypes().contains(typeKey)) {
                    Dictionary dict = type.getPanel().saveToDictionary(zone.getName());
                    if(dict != null){
