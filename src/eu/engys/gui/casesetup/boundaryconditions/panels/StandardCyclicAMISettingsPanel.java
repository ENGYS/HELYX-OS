...
import eu.engys.gui.casesetup.boundaryconditions.parameterspanel.AbstractCyclicAMISettingsPanel;
	...
	public void loadFromPatches(Patch... patches) {
		if (patches.length == 1) {
			...
			if (dictionary.found(TYPE) && dictionary.lookup(TYPE).equals(CYCLIC_AMI_KEY)) {
				...
				transformBuilder.selectDictionary(new Dictionary(couplingModel.getDictionary()));
			}
		}
	}
