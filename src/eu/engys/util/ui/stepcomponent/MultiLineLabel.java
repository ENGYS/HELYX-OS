...
public class MultiLineLabel extends JComponent {
	...
	protected Vector<String> line = new Vector<String>();// text lines to
															// display
	...
	private void divideLabel() {
		...
		while (currentPos < endPos) {
			...
			// ----------------------------------------------------------------
			while ((width < maxAllowed) && (currentPos < endPos) && (labelText.charAt(currentPos) != NEW_LINE)) {
				lastPos = currentPos;
	...
	public void paintComponent(Graphics graphics) {
		...
		for (int i = 0; i < numLines; i++) {
			...
			//graphics.drawRect(x, y-lineHeight, d.width, lineHeight);
		}
	}
	...
	public Dimension getPreferredSize() {
		measure();
		return (new Dimension(maxAllowed, (numLines * (lineHeight + lineAscent + lineDescent)) + (2 * marginHeight)));
	}
