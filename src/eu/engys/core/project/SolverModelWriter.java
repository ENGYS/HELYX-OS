...
public class SolverModelWriter {
    ...
    public SolverModelWriter(Model model) {
        this.model = model;
    }

    void save() {
        SshParameters sshParameters = model.getSolverModel().getSshParameters();
        encryptPassword(sshParameters);
        RunDict runDict = new RunDict(model.getSolverModel());
        model.getProject().getSystemFolder().setRunDict(runDict);
