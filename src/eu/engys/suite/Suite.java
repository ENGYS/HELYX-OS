...
public class Suite {
    ...
    public void batch(Arguments arguments) {
//		if (applications.size() == 1) {
			try {
				ApplicationLauncher application = applications.iterator().next();
				application.batch(arguments);
			} catch (Exception e) {
				e.printStackTrace();
				System.exit(-1);
			}
...
//		}
    ...
    public void launch(final Arguments arguments) {
    	SwingUtilities.invokeLater(new Runnable() {
			...
			public void run() {
				if (applications.size() == 1 || arguments.baseDir != null) {
					...
					JFrame frame = createAndShowFrame(arguments);
    ...
    protected JFrame createAndShowFrame(Arguments arguments) {
        JFrame frame = new JFrame(product);
        List<Action> actions = createActions(arguments);
    ...
    private List<Action> createActions(final Arguments arguments) {
